...
Modified for local use within the Agent project.
"""
...
SKILL_DIR = TOOL_DIR
HOST_TEMPLATE = TOOL_DIR / "export_host.html"
NODE_MODULES_BIN = TOOL_DIR / "node_modules" / ".bin"
...
def ensure_agent_browser() -> str:
    """查找 agent-browser 可执行文件，优先使用本地 node_modules 中的版本"""
    # 1. 优先使用本地 node_modules/.bin/ 中的版本
    if NODE_MODULES_BIN.exists():
        for candidate in ["agent-browser.cmd", "agent-browser"]:
            local_bin = NODE_MODULES_BIN / candidate
            if local_bin.is_file():
                try:
                    version = read_agent_browser_version(str(local_bin))
                    if version >= MIN_AGENT_BROWSER_VERSION:
                        log(f"agent-browser (local) version: {'.'.join(map(str, version))}")
    ...
    if executable:
        try:
            ...
            if version >= MIN_AGENT_BROWSER_VERSION:
                log(f"agent-browser (global) version: {'.'.join(map(str, version))}")
                return executable
        except Exception:
            pass
    ...
    if not npm:
        raise ExportError("npm is required but not found on PATH")

    log("installing agent-browser@latest via npm...")
    process = subprocess.run(
        [npm, "install", "--prefix", str(TOOL_DIR), "agent-browser@latest"],
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        timeout=300,
    ...
    raise ExportError("agent-browser installed but executable not found in node_modules/.bin/")
...
class BrowserSession:
    ...
    def open(self, url: str) -> None:
        self.run(
            ["--download-path", str(self.download_dir), "open", url], timeout=180
        )
