...
class PersonalityState:
    ...
    def load(self) -> dict:
        ...
        if decrypted is None:
            ...
            self._loaded = True
            self._save()
            self._log_event("tamper_detected", {}, "人格数据签名校验失败，已重置为默认值")
            return self.get_state()

        try:
            data = json.loads(decrypted)
            raw = {k: data[k] for k in DIMENSION_KEYS if k in data} if isinstance(data, dict) else {}
            for key in DIMENSION_KEYS:
                if key not in raw:
                    raw[key] = DEFAULT_PERSONALITY[key]
            self._state = raw
